// Cognito login without an SDK: InitiateAuth is a plain JSON POST. Tokens live
...
const KEY = "medscribe.doctor.session";
...
export async function login(email: string, password: string): Promise<void> {
  ...
  const r = await initiateAuth("USER_PASSWORD_AUTH", { USERNAME: email, PASSWORD: password });
  save({ idToken: r.IdToken, refreshToken: r.RefreshToken ?? "", email, exp: decodeExp(r.IdToken) });
}
